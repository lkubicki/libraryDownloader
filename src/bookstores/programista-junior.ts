...
export class ProgramistaJunior extends Bookstore {
            ...
            form:
                {
                    ...
                    redirect_to: "https://programistajr.pl/wp-admin/",
                    instance: "",
                    action: "login"
                }
        };
    ...
    private getIssueTitle($: any, magazineElement: any): { oldName: string, correctedName: string } {
        for (let element of $("tr td a", magazineElement)) {
            try {
                ...
                if (/^Programista Junior [0-9]+\/[0-9]+/.test(elementText)) {
                    return {
    ...
    private createIssueName(elementText: string): string {
        const matchArray = elementText.match(/^(Programista\sJunior) ([0-9]+)\/([0-9]+)/);
